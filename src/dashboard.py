...
<section class="chat"><h2>Chat / Control Console</h2><div class="sub" style="margin-bottom:12px">Send a prompt to one OpenPiBot agent or broadcast to all online agents. API keys stay on the Pi dashboard server. Context is saved per session and per agent.</div><div class="chat-controls"><input id="chatSession" placeholder="session id" /><button onclick="newSession()">New session</button><button onclick="clearDisplayedChat()">Clear window</button><label><input type="checkbox" id="clearContext"> Clear context on send</label></div><div class="chat-row"><select id="chatTarget"><option value="all">All agents</option></select><textarea id="chatMessage" placeholder="Type a message. Enter sends; Shift+Enter adds a line..."></textarea><button id="chatSend" onclick="sendChat()">Send</button><button id="chatStop" onclick="stopChat()" disabled>Stop</button></div><div class="conversation" id="conversation"></div><div class="chat-results" id="chatResults"></div></section>
...
function render(data){
 ...
 const opts=['<option value="all">All agents</option>'].concat((data.endpoints||[]).map(e=>`<option value="${esc(e.id)}">${esc(e.name||e.id)}</option>`));
 sel.innerHTML=opts.join(''); if([...sel.options].some(o=>o.value===current)) sel.value=current; sel.onchange=()=>loadSessionTranscript();
...
async function loadSessionTranscript(){
 const sid=getSessionId(); const target=document.getElementById('chatTarget')?.value||'all';
 try{const r=await fetch('/api/session?session_id='+encodeURIComponent(sid)+'&target='+encodeURIComponent(target),{cache:'no-store'}); const data=await r.json();
...
async function sendChat(){
 ...
 btn.disabled=true; stop.disabled=false; btn.textContent='Sending…'; out.innerHTML='<div class="reply"><div class="rhead">Waiting for response...</div><div class="rbody">Target: '+esc(target)+' • Session: '+esc(session_id)+'</div></div>';
 try{
  const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target,message,session_id,clear_context,request_id})}); const data=await r.json();
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 if(target){target.addEventListener('change',()=>loadSessionTranscript());}
...
def safe_session_id(value: str | None) -> str:
    ...
    if not raw:
        raw = f"session-{int(time.time() * 1000)}"
    safe = re.sub(r"[^A-Za-z0-9_.-]", "-", raw)[:80].strip(".-")
    return safe or f"session-{int(time.time() * 1000)}"
...
def session_transcript(session: dict[str, Any], endpoint_id: str | None = None) -> list[dict[str, Any]]:
    if endpoint_id:
        ids = [endpoint_id]
    else:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path.startswith("/api/session"):
            ...
            payload = json.dumps({"ok": True, "session_id": session.get("session_id"), "transcript": session_transcript(session, target if target and target != "all" else None)}, default=str).encode()
            self._send(payload, "application/json")
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if target and target != "all":
                endpoints = [ep for ep in endpoints if ep.get("id") == target]
            if not endpoints:
            ...
            log_chat_event({"chat_id": chat_id, "session_id": session_id, "type": "request", "target": target or "all", "message": message, "endpoint_ids": [ep.get("id") for ep in endpoints]})
            ...
            with SESSION_LOCK:
                ...
                save_session(session)
                transcript = session_transcript(session, target if target and target != "all" else None)
            log_chat_event({"chat_id": chat_id, "session_id": session_id, "type": "response", "target": target or "all", "count": len(results), "results": results})
            body = json.dumps({"ok": True, "session_id": session_id, "request_id": request_id, "target": target or "all", "count": len(results), "results": results, "transcript": transcript}, default=str).encode()
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Cache-Control", "no-store")
            self.send_header("Content-Length", str(len(body)))
